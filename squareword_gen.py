...
def GetWorkingWords(freq_csv_file, scrabble_words_file, top_n, word_len):
  ...
  print("Picked the top %d scrabble words. Here's some of the top/bottom ones:" % (len(working_words)))
...
def GenSquares(word_trie, start_word):
  ...
  for c in start_word:
    ...
    if c not in word_trie:
      log.log(" --> NO MATCH FOR TOP START WORD %s of %s" %(c, start_word))
      return
...
def main():
  ...
  parser.add_argument('--freq_csv_file', type=str, default='unigram_freq.csv')
